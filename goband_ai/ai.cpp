...
* @Last Modified time: 2017-10-29 17:29:29
*/
#include <stdio.h>
#include <stdlib.h>
#include <cstring>
...
// weight_point * get_candidates(board &b, int & num)
// {
// 	weight_point * candidates = new weight_point[BOARD_SIZE*BOARD_SIZE];
// 	num = 0;
...
// 				candidates[num++] = weight_point(x,y,b.consider_weight[x][y]);
// 				//candidates.push_back(np);
// 			}
// 		}
// 	//sort(candidates.begin(),candidates.end(),cmp_weight_point);
// 	sort(candidates,candidates+num,cmp_weight_point);
// 	return candidates;
// }

std::vector<weight_point> get_candidates(board &b)
{
...
int ai::max_value(board &b, int &alpha, int &beta, int &next_x, int &next_y, int depth)
{
	...
	std::vector<weight_point> candidates = get_candidates(b);
	// for(int x=1;x<=BOARD_SIZE;++x)
	// 	for(int y=1;y<=BOARD_SIZE;++y)
	// 	{
	// 		if(b.consider_weight[x][y]!=0 && b.is_empty(x,y))
	...
	//weight_point *candidates = get_candidates(b,num_of_candidates);
